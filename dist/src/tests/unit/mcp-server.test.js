...
describe('MCPPortalServer', () => {
    ...
    beforeEach(() => {
        ...
        SwaggerLoader.mockImplementation(() => mockSwaggerLoader);
        Authentication.mockImplementation(() => mockAuth);
    });
    describe('constructor', () => {
        it('should initialize with correct default values', () => {
            server = new MCPPortalServer();
            expect(Logger).toHaveBeenCalledWith('info');
            expect(SwaggerLoader).toHaveBeenCalledWith('https://api.portaldatransparencia.gov.br/swagger-ui/swagger.json', mockLogger, {} // Empty auth headers when no API key is provided
            );
    ...
    describe('initialize', () => {
        ...
        it('should initialize successfully with valid swagger spec', async () => {
            ...
            mockSwaggerLoader.loadSpec.mockResolvedValue(mockSpec);
            mockAuth.hasApiKey.mockReturnValue(false);
            await server.initialize();
            expect(mockSwaggerLoader.loadSpec).toHaveBeenCalled();
            expect(mockLogger.info).toHaveBeenCalledWith('Initializing MCP Portal da Transparência server...');
        ...
        it('should handle API key validation errors', async () => {
            ...
            await server.initialize();
            expect(mockLogger.warn).toHaveBeenCalledWith('⚠️ Could not validate API key due to network error');
            expect(mockLogger.info).toHaveBeenCalledWith('🔄 Will proceed with initialization - test your key later');
        });
    });
