...
const mockedSwaggerParser = SwaggerParser;
// Mock Logger
...
describe('SwaggerLoader', () => {
    ...
    describe('loadSpec', () => {
        it('should load and validate swagger specification successfully', async () => {
            ...
            mockedSwaggerParser.validate.mockResolvedValue(mockSpec);
            // Act
        ...
        it('should handle axios errors', async () => {
            ...
            // Act & Assert
            await expect(swaggerLoader.loadSpec()).rejects.toThrow('Failed to load Swagger specification: Network error');
        ...
        it('should handle swagger validation errors', async () => {
            ...
            // Act & Assert
            await expect(swaggerLoader.loadSpec()).rejects.toThrow('Failed to load Swagger specification: Invalid swagger spec');
            expect(mockLogger.error).toHaveBeenCalledWith('Failed to load Swagger specification', {
    ...
    describe('validateSpecStructure', () => {
        it('should validate valid OpenAPI spec', () => {
            // Act
            const isValid = swaggerLoader.validateSpecStructure(mockSpec);
            // Assert
            expect(isValid).toBe(true);
            expect(mockLogger.info).toHaveBeenCalledWith('Swagger specification structure validation passed');
        });
        it('should reject spec without version', () => {
            // Arrange
            const invalidSpec = { ...mockSpec };
            delete invalidSpec.openapi;
            // Act
            const isValid = swaggerLoader.validateSpecStructure(invalidSpec);
            // Assert
            expect(isValid).toBe(false);
            expect(mockLogger.error).toHaveBeenCalledWith('Swagger specification structure validation failed', { error: 'Missing OpenAPI/Swagger version' });
        });
        it('should reject spec without info section', () => {
            // Arrange
            const invalidSpec = { ...mockSpec };
            delete invalidSpec.info;
            // Act
            const isValid = swaggerLoader.validateSpecStructure(invalidSpec);
            // Assert
            expect(isValid).toBe(false);
            expect(mockLogger.error).toHaveBeenCalledWith('Swagger specification structure validation failed', { error: 'Missing info section' });
        });
        it('should reject spec without paths section', () => {
            // Arrange
            const invalidSpec = { ...mockSpec };
            delete invalidSpec.paths;
            // Act
            const isValid = swaggerLoader.validateSpecStructure(invalidSpec);
            // Assert
            expect(isValid).toBe(false);
            expect(mockLogger.error).toHaveBeenCalledWith('Swagger specification structure validation failed', { error: 'Missing paths section' });
        });
    });
...
//# sourceMappingURL=SwaggerLoader.test.js.map